...
def remove_deadend(v, maze, cheese):
    if v in maze.keys() and v not in cheese:
        neighbours = list(maze[v].keys())
        if len(neighbours) == 1:
            del maze[v]
            del maze[neighbours[0]][v]
            remove_deadend(neighbours[0],maze,cheese)
...
def preprocessing ( maze:             Union[numpy.ndarray, Dict[int, Dict[int, int]]],
    ...
    """
        ...
        Out:
            * None.
    """
    vertices = list(maze.keys())
    for v in vertices:
        remove_deadend(v,maze,cheese)


    # To store the already visited cells
    memory.visited_cells = []
    # To store the player's trajectory
    memory.trajectory = []
...
def turn ( maze:             Union[numpy.ndarray, Dict[int, Dict[int, int]]],
    ...
    # If there is no unvisited neighbor, choose one randomly
        ...
        neighbor = memory.trajectory.pop()
