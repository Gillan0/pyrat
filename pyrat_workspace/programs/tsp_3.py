...
def tsp_3 ( complete_graph: Dict[int, Dict[int, int]],
          source:         int
        ) ->              Tuple[List[int], int]:
    """
    ...
    def _tsp_3(current_vertex, current_length,current_route):
        # Appel des variables globales best_route et best_length
        global best_route, best_length
        """
        DESCRIPTION
        ...
        if current_length < best_length:
            ...
            if len(current_route) == nb_vertices :
                ...
                # On trie les voisins par poids croissant
                L = complete_graph[current_vertex].keys()
                L = sorted(L, key = lambda el : complete_graph[current_vertex][el])

                # Parcours de tous les voisins non explorés
                for neighbour in L:
                    if neighbour not in current_route:
                        _tsp_3(neighbour, current_length + complete_graph[current_vertex][neighbour], current_route + [neighbour])

    _tsp_3(source,0,[source])
...
def preprocessing ( maze:             Union[numpy.ndarray, Dict[int, Dict[int, int]]],
    ...
    complete_route = tsp_3(meta_graph, starting_point)[0]
...
assert(tsp_3(meta_graph_test_tsp_1, 0) == ([0,2,4,1,3],14))
assert(tsp_3({0:{}}, 0) == ([0],0))
