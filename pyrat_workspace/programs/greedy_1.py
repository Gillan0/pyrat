...
def greedy ( graph:          Union[numpy.ndarray, Dict[int, Dict[int, int]]],
    ...
    """
    def _greedy(graph, current_vertex, vertices, route):
        if vertices == []:
            return route
        else:
            scores, routing_table = give_score(graph, current_vertex, vertices)

            vertices.sort(key = lambda tmp : scores[tmp], reverse = True)
            next_vertex : int = vertices.pop()

            return _greedy(graph, next_vertex, vertices, route + find_route(routing_table, current_vertex, next_vertex))


    return _greedy(graph, initial_vertex, vertices, [initial_vertex])
...
def preprocessing ( maze:             Union[numpy.ndarray, Dict[int, Dict[int, int]]],
    ...
    vertices : List[int] = [starting_point] + cheese

    # Obtention du chemin
    route = greedy(maze, starting_point, vertices)
