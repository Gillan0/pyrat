...
def tsp_1 ( complete_graph: Dict[int, Dict[int, int]],
          source:         int
        ) ->              Tuple[List[int], int]:
    """
    ...
    def _tsp_1(current_vertex, current_length,current_route):
        # Appel des variables globales best_route et best_length
        global best_route, best_length
        """
        DESCRIPTION
        ...
        if len(current_route) == nb_vertices :
            # Si le chemin actuel est plus court que celui que le meilleur, on le remplace
            ...
            for neighbour in complete_graph[current_vertex].keys():
                if neighbour not in current_route:
                    _tsp_1(neighbour, current_length + complete_graph[current_vertex][neighbour], current_route + [neighbour])

    _tsp_1(source,0,[source])
...
assert(tsp_1(meta_graph_test_tsp_1, 0) == ([0,2,4,1,3],14))
assert(tsp_1({0:{}}, 0) == ([0],0))
